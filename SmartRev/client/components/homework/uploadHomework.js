...
export default function uploadHomework({navigation}) {
  ...
  const[fileList, setFileList] = useState([]);
    ...
    function saveFileToRealimeDatabase(downloadURL, file){
      ...
      database().ref('Homework/' + uniqueKey).update({
        fileName: file.name,
        fileType: file.type,
        fileURL: downloadURL,
      })
    ...
    useEffect(() => {
      ...
      const onChildAdded = database().ref('Homework').on('child_added',(snapshot)=>{
        let helperArr = [];
        helperArr.push(snapshot.val());
        setFileList((files)=>[...files,...helperArr])
        
      })
      return()=> database().ref('Homework').off('child_added', onChildAdded);
      }, []);
